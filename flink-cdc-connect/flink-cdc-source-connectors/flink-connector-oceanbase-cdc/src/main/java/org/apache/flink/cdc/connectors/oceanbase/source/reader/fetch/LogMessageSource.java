...
public class LogMessageSource
    ...
    private void handleTransactionBegin(
        ...
        if (offset.getCommitVersion() != null) {
            if (!offset.getCommitVersion().equals(offsetContext.getCommitVersion())) {
                offsetContext.setCommitVersion(
                        Instant.ofEpochSecond(Long.parseLong(offset.getTimestamp())),
                        offset.getCommitVersion());
            } else {
                offsetContext.commitVersionAccumulate();
            }
        }
